...
import { Bell, Check, ChevronLeft, ChevronRight, CircleDot, Import, MessageSquarePlus, Plus, Sparkles, Trash2, X } from 'lucide-react';
...
export function SettingsDrawer({ config, alerts, onToggleAlerts, onSave, onTest, onClose }: { config: ProviderConfig; alerts: boolean; onToggleAlerts(): void; onSave(config: ProviderConfig): void; onTest(endpoint: string): Promise<string[]>; onClose(): void }) {
  ...
  return <section className="drawer"><button className="drawer-close" onClick={onClose}><X size={16}/></button><div className="field"><h2>Ollama connection</h2><p>Haru will be able to talk to Ollama on your machine. Provider credentials will stay outside the renderer.</p><div className="form-grid"><input value={endpoint} onChange={event => setEndpoint(event.target.value)} placeholder="http://localhost:11434"/><input value={model} onChange={event => setModel(event.target.value)} placeholder="qwen3:8b"/></div>{status.state !== 'idle' && <p className={status.state === 'error' ? 'status-error' : 'status-ok'}>{status.state === 'testing' ? 'Testing…' : status.message}</p>}</div><div className="field"><h2>Reminder alerts</h2><p>A desktop notification when a kept reminder or appointment comes due. Items with no time of their own arrive at 9am.</p><button className={alerts ? 'ghost on' : 'ghost'} onClick={onToggleAlerts} aria-pressed={alerts}><Bell size={13}/> {alerts ? 'Alerts on' : 'Alerts off'}</button></div><div className="drawer-foot"><button className="ghost" onClick={test} disabled={status.state === 'testing'}>{status.state === 'testing' ? 'Testing…' : 'Test connection'}</button>{saved && <span className="saved"><Check size={13}/> Saved</span>}<button className="solid" onClick={save}>Save setup</button></div></section>;
}
